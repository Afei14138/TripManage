...
<meta http-equiv="content-type" content="text/html; charset=utf-8"> 
  ...
  <body>
   <div style="margin:20px 0;"></div>
	...
	<div id="dlg" class="easyui-dialog" title="打回理由" data-options="iconCls:'icon-save'" closed = "true" style="width:800px;height:350px;padding:40px">
	<form id="ff" class="easyui-form" method="post" data-options="novalidate:true">
	    	<table cellpadding="5">
					...
					<tr>
						...
						<td>
							<input class="easyui-textbox" type="text" name="approve_depmanager"
					...
					<tr>
						...
						<td>
							<input class="easyui-textbox" type="text" name="return_reason_depmanager"
    ...
    <script type="text/javascript">
       $(function(){
		    ...
		    columns:[[
				...
				{field:'option',align:'left',title:'操作',width:65,
					formatter:function(value,row,index){
						if(row.state !="部门经理通过" && row.state !="部门经理打回"&&row.state !="总经理通过"&&row.state !="总经理拒绝" ){
							var a = "<a href=\"javascript:agree(\'"+row.trapp_no+"\');\">通过</a>  "; 
								a += "<a href=\"javascript:reason(\'"+row.trapp_no+"\');\">打回</a>  "; 
							return a ;
						}
					}
				}
		...
		console.log("trapp_no:"+trapp_no);
			$.ajax({
		        type: "post",
		        url: "ApproveBorrowServlet?action=agree&trapp_no="+trapp_no,
		        success: function()
		        {
       ...
       $('#ff').form('submit',{
				...
				url:'ApproveBorrowServlet?action=updateApp',//把表单提交到它
	...
	/*	function disagree(trapp_no){
		
			$.ajax({
		        type: "post",
		        url: "ApproveBorrowServlet?action=disagree&trapp_no="+trapp_no,
		        success: function(){
							alert("处理成功");
