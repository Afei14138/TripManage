  ...
  <body>
	...
	<table id="tt"></table>
	 ...
	 <div id="dlg" class="easyui-dialog" title="打回理由" data-options="iconCls:'icon-save'" closed = "true" style="width:1000px;height:400px;padding:40px">
	    	    ...
	    	    <tr>
	    		 ...
	    		 <tr>
	    			<td>审批部门经理:</td>
	    			<td><input class="easyui-textbox" name="approve_depmanager" type="text" data-options="required:true"></input></td>
	    		</tr>
                 <tr>
	    			<td>打回理由:</td>
	    			<td><input class="easyui-textbox" name="return_reason_depmanager" data-options="multiline:true" style="height:100px;width: 400px"></input></td>
	    		</tr>
	    		</table>
	   ...
	   <div id="rfdlg" class="easyui-dialog" title="拒绝理由" data-options="iconCls:'icon-save'" closed = "true" style="width:1000px;height:400px;padding:40px">
	    	    ...
	    	    <table cellpadding="5">
                 ...
                 <tr>
	    			<td>拒绝理由:</td>
	    			<td><input class="easyui-textbox" name="refuse_reason_depmanager" data-options="multiline:true" style="height:100px;width: 400px"></input></td>
	    		</tr>
	    		</table>
	    		<div style="text-align:center;padding:5px">
    ...
    <script type="text/javascript">
       $(function(){
       	...
       	$('#tt').datagrid({
		    columns:[[
				...
				{field:'plan_time',width:100,aalign:'left',title:'计划用时'},
				...
				{field:'option',align:'left',title:'操作',width:140,align:'center',
					formatter:function(value,row,index){
						if(row.state !="部门经理通过" && row.state !="部门经理打回"&&row.state !="部门经理拒绝"){
		...
		function agree(no){
			$.ajax({
		        type: "post",
		        url: "ApproveTravelServlet?action=agree&no="+no,
		        success: function(){
       ...
       function disagree(no){
            ...
            $.ajax({
		        type: "post",
		        url: "ApproveTravelServlet?action=disagree&no="+no,
		    });
		 };
		...
		function refuse(no){
		  $("#rfdlg" ).dialog("open");
		   document.getElementById("newno").value=no;
			$.ajax({
		        type: "post",
		        url: "ApproveTravelServlet?action=refuse&no="+no,
		    });
		};
		function submitForm(){
			$('#fff').form('submit',{
				...
				url:'ApproveTravelServlet?action=updateApproveRf',
		...
		function submit(){
			$('#ff').form('submit',{
				...
				url:'ApproveTravelServlet?action=updateApproveDis',
